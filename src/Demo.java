...
public class Demo {
	...
	public static void main (String [] args){
		...
		System.out.println("Hey! Enter in a file :D ");
	...
	public static ArrayList<String> traverse(int[][] matrix){
		...
		while(!trav.isEmpty()){
			...
			for(int i=0; i<matrix.length; i++){
				...
				//if the value in the matrix is a 1, and the value has not been added to the array or queue.
				if(matrix[ret.get(ret.size()-1).charAt(0)-65][i] == 1 && !ret.contains(String.valueOf(Character.toChars(i+65)[0])) && trav.positionOf(String.valueOf(Character.toChars(i+65)[0])) ==-1){
					//add it to the queue
